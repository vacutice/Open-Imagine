...
function i(){
for(var e=window.pageYOffset||document.documentElement.scrollTop,t=c.video_top.length,n=e+d.getInnerHeight(),r=0,a=0;t>a;a++){
var m=c.video_top[a];
m.reported?r++:n>=m.start&&n<=m.end&&(m.reported=!0,setTimeout(function(e,t,i){
return function(){
...
video_iframe:[],
video_top:[]
},a=e("pages/version4video.js"),m=e("biz_common/dom/attr.js"),p=(m.setProperty,e("biz_common/dom/event.js")),l=document.getElementsByTagName("iframe"),_=[];
/MicroMessenger/.test(navigator.userAgent);
}
window.reportVid=[];
for(var u=Math.ceil(1e4*Math.random()),w=0,g=l.length;g>w;++w)!function(e){
...
s=s.replace(/&amp;/g,"&");
for(var m=s.split("&"),p=["/mp/newappmsgvote?action=show"],l=0;l<m.length;l++)(0==m[l].indexOf("__biz=")||0==m[l].indexOf("supervoteid="))&&p.push(m[l]);
p.length>1&&(i=p.join("&")+"#wechat_redirect");
}
}
if(d&&(c.txVideoReg.test(d)||c.mpVideoReg.test(d))){
if(a.isShowMpVideo()||c.mpVideoReg.test(d)){
var w=r.getQuery("vid",i);
if(!w)return;
var g=e.getAttribute("data-vw"),v=e.getAttribute("data-vh"),f=document.domain;
"qq.com"==f&&((new Image).src="https://badjs.weixinbridge.com/badjs?id=139&level=4&from="+window.encodeURIComponent(window.location.host)+"&msg="+window.encodeURIComponent(window.location.href),
...
dom:e,
vid:w
}),d=["/mp/videoplayer?video_h=",v,"&video_w=",g,"&scene=",window.source,"&random_num=",u,"&article_title=",encodeURIComponent(window.msg_title.htmlDecode()),"&source=4&vid=",w,"&mid=",appmsgid,"&idx=",itemidx||idx,"&__biz=",biz,"&nodetailbar=",window.is_temp_url?1:0,"&uin=",uin,"&key=",key,"&pass_ticket=",pass_ticket,"&version=",version,"&devicetype=",window.devicetype||"","&wxtoken=",window.wxtoken||"","&sessionid=",window.sessionid||"","&preview=",window.is_temp_url?1:0].join(""),
...
window.__videohook__=1,i=!1,o=!1,d={},t.onload=function(){
console.log("4",Date.now()),i=!0,r(t,d,i,o);
},console.log("1",Date.now()),t.setAttribute("src",e),console.log("2",Date.now()),
n({
url:e,
type:"GET",
f:"json",
success:function(n){
console.log("3",Date.now()),o=!0;
try{
d=JSON.parse(n),r(t,d,i,o);
}catch(s){
r(t,d,i,o);
}
window.resp=n,t.setAttribute("data-realsrc",e),t.contentWindow.__iframe_src__=e;
}
});
}();
...
var r=e[o];
if(!r||!r.dom)return;
for(var s=r.dom,a=s.offsetHeight,m=0;s&&t!==s;)m+=s.offsetTop,s=s.offsetParent;
c.video_top.push({
start:m+a/2,
end:m+a/2+d.getInnerHeight(),
reported:!1,
vid:r.vid
});
}
